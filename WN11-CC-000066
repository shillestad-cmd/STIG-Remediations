<#
.SYNOPSIS
    This PowerShell script enables command-line details in process creation audit events.
...
.USAGE
    ...
    Process creation events also require Audit Process Creation - Success
    (WN11-AU-000050) and the subcategory override policy (WN11-SO-000030).
#>
...
# Display the configured value. Expected output: 1.
Get-ItemPropertyValue -Path $RegPath -Name 'ProcessCreationIncludeCmdLine_Enabled'
